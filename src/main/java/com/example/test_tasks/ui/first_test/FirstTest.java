...
import com.example.test_tasks.service.SubstringService;
...
public class FirstTest extends VerticalLayout {
    ...
    private final Button go, latestResults, mainPage;
    ...
    public FirstTest(SubstringService subStringService) {
        ...
        go = new Button("Go");
        latestResults = new Button("Results");
        mainPage = new Button("Home");
        latestResults.addClickListener(e -> UI.getCurrent().navigate("/substring/results"));
        ...
        horizontalLayout = new HorizontalLayout(go, latestResults);
        add(title, substringsArea, sourceStringsArea, result, horizontalLayout, mainPage);
    }
